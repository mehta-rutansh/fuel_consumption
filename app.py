...
/* Emission badge */
.badge-green  { background:#276749; color:#9ae6b4; padding:6px 16px; border-radius:20px; font-weight:600; font-size:0.85rem; }
.badge-blue   { background:#2a4365; color:#90cdf4; padding:6px 16px; border-radius:20px; font-weight:600; font-size:0.85rem; }
.badge-yellow { background:#744210; color:#fbd38d; padding:6px 16px; border-radius:20px; font-weight:600; font-size:0.85rem; }
.badge-orange { background:#7b341e; color:#fbd38d; padding:6px 16px; border-radius:20px; font-weight:600; font-size:0.85rem; }
.badge-red    { background:#742a2a; color:#fc8181; padding:6px 16px; border-radius:20px; font-weight:600; font-size:0.85rem; }
...
with st.sidebar:
    ...
    page = st.radio("", ["🏠  Home", "📊  EDA Charts", "🤖  Predict CO₂",  "📁  Bulk Scanner"], label_visibility="collapsed")
    st.markdown("---")
    st.markdown("""
    ...
    Compact/Subcompact = Hatchback/Small sedan<br>
    Mid-size = Family sedan (Dzire, City)<br>
    Full-size = Large sedan (Camry, Accord)<br>
...
if page == "🏠  Home":
    ...
    avg_kmpl = round(100 / df['COMB (L/100 km)'].mean() * 1.609, 1)  # mpg to km/L approx
...
# ══════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        with c1:
            ...
            ax.axvline(df['EMISSIONS'].mean(), color='#f6e05e', linestyle='--', linewidth=1.5, label=f"Avg: {df['EMISSIONS'].mean():.0f}")
            ax.legend(fontsize=9)
            ax.grid(True, alpha=0.3)
            fig.tight_layout()
            st.pyplot(fig)
            plt.close()
            st.markdown("<p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>Most vehicles emit 200–300 g/km. Yellow line = average.</p>", unsafe_allow_html=True)
        ...
        with c2:
            ...
            ax.set_ylabel('Number of Vehicles')
            ax.axvline(df['COMB (L/100 km)'].mean(), color='#f6e05e', linestyle='--', linewidth=1.5, label=f"Avg: {df['COMB (L/100 km)'].mean():.1f}")
            ax.legend(fontsize=9)
            ax.grid(True, alpha=0.3)
            fig.tight_layout()
            st.pyplot(fig)
            plt.close()
            st.markdown("<p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>8 L/100km = 12.5 km/L mileage (like a Maruti Swift)</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with c1:
            ...
            st.pyplot(fig)
            plt.close()
            st.markdown("<p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>Indian cars mostly have 1.0–2.0L engines</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with c2:
            ...
            bars = ax.bar(cyl_counts.index, cyl_counts.values,
                          color=['#63b3ed','#4fd1c5','#68d391','#f6ad55','#fc8181','#b794f4','#76e4f7'],
            ...
            st.pyplot(fig)
            plt.close()
            st.markdown("<p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>4-cylinder engines dominate (like most Indian cars)</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        with c1:
            ...
            plt.colorbar(sc, ax=ax, label='Cylinders')
            ...
            st.pyplot(fig)
            plt.close()
            st.markdown("<p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>Bigger engine → more CO₂. Color shows cylinder count.</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with c2:
            ...
            st.pyplot(fig)
            plt.close()
            st.markdown("<p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>Highway driving always uses less fuel than city driving</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        for patch, color in zip(bp['boxes'], palette[:len(cyl_vals)]):
        ...
        st.pyplot(fig)
        plt.close()
        st.markdown("<p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>More cylinders = higher CO₂. 4-cylinder cars (most Indian cars) are in the middle range.</p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tab3:
        ...
        with c1:
            ...
            st.pyplot(fig)
            plt.close()
            st.markdown("<p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>Regular Petrol (X) is most common — same as India</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with c2:
            ...
            st.pyplot(fig)
            plt.close()
            st.markdown("<p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>Green = below average. CNG has lowest CO₂ emissions.</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        st.pyplot(fig)
        plt.close()
        st.markdown("<p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>CO₂ emissions have reduced over the years due to better engine technology</p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tab4:
        ...
        mask[np.triu_indices_from(mask, k=1)] = True  # only lower triangle
        ...
        plt.close()
        st.markdown("""
        <p style='color:rgba(255,255,255,0.45); font-size:0.78rem; margin-top:6px;'>
...
# ══════════════════════════════════════════════════════════════
    ...
    • <b>Combined</b> = Average of both = ~6.5 L/100km for Swift → mileage = 100/6.5 = <b>15.4 km/L</b><br>
    ...
    with col1:
        ...
        year = st.number_input(
            "Vehicle Model Year",
            min_value=2000, max_value=2022, value=2018, step=1
        )
    ...
    with col2:
        ...
        # Show km/L equivalent live
        st.markdown(f"<p style='color:#68d391; font-size:0.85rem; margin-top:-10px;'>↳ City mileage: <b>{100/city_fc:.1f} km/L</b></p>", unsafe_allow_html=True)
        ...
        st.markdown(f"<p style='color:#68d391; font-size:0.85rem; margin-top:-10px;'>↳ Highway mileage: <b>{100/hwy_fc:.1f} km/L</b></p>", unsafe_allow_html=True)

        comb_fc = round((city_fc + hwy_fc) / 2, 1)
        comb_mpg = int(235.21 / comb_fc)  # L/100km to mpg conversion

        st.markdown(f"""
        <div style='background:rgba(99,179,237,0.1); border-radius:10px; padding:12px 16px; margin-top:8px;'>
            <span style='color:rgba(255,255,255,0.6); font-size:0.8rem;'>Auto-calculated Combined</span><br>
            <span style='color:#63b3ed; font-weight:700; font-size:1.1rem;'>{comb_fc} L/100 km</span>
            <span style='color:rgba(255,255,255,0.4); font-size:0.8rem;'> = {100/comb_fc:.1f} km/L = {comb_mpg} mpg</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        fuel_options = {
            "Regular Petrol (Most common — Maruti, Hyundai)": 3,
            "Premium Petrol (High-end — BMW, Audi)": 4,
        ...
        trans_options = {
            "Automatic (AT)": 0,
            "Automated Manual (AMT) — like Maruti AMT": 1,
            "Automatic with Select Shift (AT+Paddle)": 2,
            "CVT (Continuously Variable — Honda City CVT)": 3,
            "Manual (MT) — most common in India": 4,
        }
    ...
    sample = [[engine_size, cylinders, city_fc, hwy_fc, comb_fc, comb_mpg,
    ...
    if predict_btn:
        ...
        with c2:
            ...
            <div style='background:rgba(99,179,237,0.12); border:1px solid rgba(99,179,237,0.3);
                        border-radius:16px; padding:24px; text-align:center; margin-top:0;'>
                <div style='font-size:2.5rem; font-weight:700; color:#63b3ed;'>{kmpl:.1f}</div>
                <div style='color:rgba(255,255,255,0.6); font-size:0.9rem; margin-top:6px;'>km/L Mileage</div>
            </div>""", unsafe_allow_html=True)
        with c3:
            avg_co2 = df['EMISSIONS'].mean()
            diff    = result - avg_co2
            ...
            <div style='background:rgba(246,173,85,0.1); border:1px solid rgba(246,173,85,0.3);
                ...
                <div style='font-size:2rem; font-weight:700; color:{color};'>{sign}{abs(diff):.0f}</div>
                <div style='color:rgba(255,255,255,0.6); font-size:0.85rem; margin-top:6px;'>vs Dataset Avg<br>({avg_co2:.0f} g/km)</div>
            </div>""", unsafe_allow_html=True)

        # Emission rating badge
        st.markdown("<br>", unsafe_allow_html=True)
        if result < 150:
        ...
        st.markdown(f"<p style='color:rgba(255,255,255,0.55); font-size:0.85rem; margin-top:8px;'>{desc}</p>", unsafe_allow_html=True)
        ...
        <div style='margin-top:16px;'>
            <div style='color:rgba(255,255,255,0.5); font-size:0.78rem; margin-bottom:6px;'>
        ...
        # ══════════════════════════════════════════════════════════════
# PAGE 4 — BULK SCANNER  (paste this whole block as a new elif
# in app.py, and add "📁  Bulk Scanner" to the sidebar radio list)
# ══════════════════════════════════════════════════════════════
    ...
    if uploaded_file is not None:
        ...
        # ── Required fields the model needs, with keyword hints ─
        # field_key : (display label, list of keywords to fuzzy-match column names)
        required_fields = {
        ...
        def guess_column(keywords, columns):
            """Return best matching column name based on keyword overlap, or None."""
        ...
        if run_btn:
            ...
            # Engine size & cylinders are essential — everything else can be defaulted
            ...
            # ── Fuel consumption fields — derive whichever are missing ──
            ...
            # Fill in whatever is missing using whatever is available
            if not has_comb and has_city and has_hwy:
                ...
                # Nothing given at all — estimate from engine size using dataset average ratio
                work["COMB_FC"] = 4.0 + work["ENGINE SIZE"] * 1.8   # rough fallback formula

            if not has_city:
                work["CITY_FC"] = work["COMB_FC"] * 1.15   # city typically ~15% higher than combined
            if not has_hwy:
                work["HWY_FC"] = work["COMB_FC"] * 0.85    # highway typically ~15% lower than combined

            # mpg derived from combined L/100km
            ...
            if mapping["year"] != "— None / Not Available —":
                ...
                work["YEAR"] = 2015  # neutral default
            ...
            if mapping["fuel"] != "— None / Not Available —":
                ...
                work["FUEL_ENC"] = raw_fuel.map(fuel_map).fillna(3).astype(int)  # default regular petrol
            else:
                work["FUEL_ENC"] = 3  # default: regular petrol
            ...
            if mapping["vclass"] != "— None / Not Available —":
                ...
                work["CLASS_ENC"] = raw_class.map(class_map).fillna(2).astype(int)  # default mid-size
            else:
                work["CLASS_ENC"] = 2  # default: mid-size
                ...
                def map_trans(val):
                    ...
                    for key, code in trans_map.items():
                        if val.startswith(key):
                            return code
                    return 0  # default automatic
                work["TRANS_ENC"] = raw_trans.apply(map_trans)
            else:
                work["TRANS_ENC"] = 0  # default: automatic
            ...
            # ── Drop rows where essential numeric fields failed to parse ──
            ...
            # ── Build final feature matrix with EXACT column names used in training ──
            X_bulk = work[[
            ...
            # Rename to match the exact feature names the model was trained on
            ...
            # ── Build result table (original data + predictions) ──
            ...
            for c, (icon, val, label) in zip([s1, s2, s3, s4], cards):
                c.markdown(f"""
                <div class="metric-card">
                    <div class="metric-icon">{icon}</div>
                    <div class="metric-value">{val}</div>
            ...
            # Distribution chart of predictions
        ...
        <div style='text-align:center; padding:60px 20px; color:rgba(255,255,255,0.3);'>
            ...
            <div style='font-size:1rem; margin-top:10px;'>Upload a CSV or Excel file above to get started</div>
        </div>
        """, unsafe_allow_html=True)
